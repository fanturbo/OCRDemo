...
public class MainActivity extends AppCompatActivity {
    ...
    private static final int REQUEST_CODE_HAND = 125;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 手写文字识别
        findViewById(R.id.hand_button).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (!checkTokenStatus()) {
                    return;
                ...
                intent.putExtra(CameraActivity.KEY_CONTENT_TYPE,
                        CameraActivity.CONTENT_TYPE_GENERAL);
                startActivityForResult(intent, REQUEST_CODE_HAND);
            }
        });
    ...
    private void initAccessTokenWithAkSk() {
        ...
        }, getApplicationContext(), "7YEsb9xKRfGR7z5djoNTrkLs", "BchGAzLK33uzLg9GdvwE9Q4hTxmYbKja");
    }
    ...
    private void infoPopText(final String result) {
        ...
        for (Result.WordsResultBean wordsResultBean : fromJson.getWords_result()) {
            stringBuffer.append(wordsResultBean.getWords() + System.lineSeparator());
        }
        alertText("", stringBuffer.toString());

        ClipboardManager mClipboardManager = (ClipboardManager) getSystemService(CLIPBOARD_SERVICE);
        ClipData clipData = ClipData.newPlainText("copy from demo", stringBuffer);
        mClipboardManager.setPrimaryClip(clipData);
        Toast.makeText(getApplicationContext(), "返回结果已复制到剪切板，到其他地方粘贴即可", Toast.LENGTH_LONG).show();
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        // 识别成功回调，手写文字识别
        if (requestCode == REQUEST_CODE_HAND && resultCode == Activity.RESULT_OK) {
            RecognizeService.recHandWriting(FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(),
                    new RecognizeService.ServiceListener() {
                        @Override
                        public void onResult(String result) {
                            infoPopText(result);
